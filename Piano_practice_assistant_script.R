...
# The minimum number of chords required for adequate calibration is around 30 - 40. Size of training subset represents a trade-off between reduced accuracy of initial model due to over-fitting vs patience required from user to complete calibration.

# Note that success likelihood is that estimated by the logistic regression model from the (initially somewhat limited) training data it receives; it is not the true success likelihood of the user in chord playing trials. This difference is particularly clear when stochasticity leads to extreme values of predicted success likelihood.

# Initial highest chords are very high, due to small dataset / number of trials leading to stochasticity - these extreme values were brought down to more representative values following more trials.

# Lowest chords' success likelihoods initially begin very low, but increase with more trials - this represents learning.

# A rolling window could be instituted to better reflect user's current ability.
